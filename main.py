...
###
    ...
    for root, dirs, files in os.walk(backup_folder): # updating files content
        for file in all_from:
            backup_file_path = os.path.join(root, file)
            main_file_path = os.path.join(main_folder, file)

            with open(backup_file_path, 'r') as backup_file: # store the files
                backup_content = backup_file.read()

            with open(main_file_path, 'r') as main_file: # store the files
                main_content = main_file.read()

            if backup_content != main_content: # comparing the content
                main_log_message = os.path.join(main_folder, log_message)
                with open(main_log_message, "a") as log_file:
                    log_file.write(
                        f"{datenow}: File {os.path.basename(backup_file_path)} from {os.path.basename(backup_folder)} was successfully updated.\n")

                print(f"{datenow}: File {os.path.basename(backup_file_path)} from {os.path.basename(backup_folder)} was successfully updated.\n")
